...
@Slf4j
@Service
public class UseHystrixCommandAndAnnotation {

    private final NacosClientService nacosClientService;

    public UseHystrixCommandAndAnnotation(NacosClientService nacosClientService) {
    ...
    public List<ServiceInstance> getNacosClientIngo(String serviceId){
        try {
            Thread.sleep(3000);
        }catch (Exception ex){
    ...
    /**
     * getNacosClientIngo 兜底策略 - Hystrix 后备模式
     * @param serviceId
     * @return
     */
    public List<ServiceInstance> getNacosClientInfoFallback(String serviceId) {
        log.info("can not get nacos client, trigger hystrix fallback: [{}],[{}]",serviceId,Thread.currentThread().getName());
